    ...
    class GenerateParameters
    {
        ...
        public static void ReadBackCheck(Form1 fm)
        {
            ...
            try
            {
                ...
                {
                    fm.myTextBoxNUM_AutoHoldTime.Text = "2.0";
                }
                else if (Convert.ToDouble(fm.myTextBoxNUM_AutoHoldTime.Text) > (12))
                {
                    fm.myTextBoxNUM_AutoHoldTime.Text = "12.0";
                }
            }
            catch (Exception)
            {
                fm.myTextBoxNUM_AutoHoldTime.Text = "8.0";
            }

            // 手动巡航按键保持时间
            try
            {
                if (Convert.ToDouble(fm.myTextBoxNUM_ManualHoldTime.Text) < (0.1))
                {
                    fm.myTextBoxNUM_ManualHoldTime.Text = "0.1";
                }
                else if (Convert.ToDouble(fm.myTextBoxNUM_ManualHoldTime.Text) > (4))
                {
                    fm.myTextBoxNUM_ManualHoldTime.Text = "4.0";
                }
            }
            catch (Exception)
            {
                fm.myTextBoxNUM_ManualHoldTime.Text = "2.0";
            }
