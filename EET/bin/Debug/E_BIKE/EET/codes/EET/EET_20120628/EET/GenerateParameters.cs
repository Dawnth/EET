...
namespace EET
{
    class GenerateParameters
    {
        ...
        public static void InitParameters(Form1 fm)
        {
            ...
            // 36V电池过压值
            Form1.gss_BatteryMaximumVoltage_36V = "137";
            // 36V电池欠压值
            Form1.gss_BatteryMinimumVoltage_36V = "100";
        }
        ...
        public static void UIToParameters(Form1 fm)
        {
            ...
            { 
                // 电池过压值=BusVolt（写入值） × 0.0653 × 255 / 5
                Form1.gss_BatteryMaximumVoltage_36V = Convert.ToInt32(Convert.ToDouble(fm.myTextBoxNUM_BatteryMaximumVoltage.Text) * 0.0653 * 255 / 5).ToString();
                // 电池欠压值=BusVolt（写入值） × 0.0653 × 255 / 5
                Form1.gss_BatteryMinimumVoltage_36V = Convert.ToInt32(Convert.ToDouble(fm.myTextBoxNUM_BatteryMinimumVoltage.Text) * 0.0653 * 255 / 5).ToString();
            }
            // 限速线分压系数=写入值 × 10
            Form1.gss_SpeedLimitVoltageDivideCoefficient = "17";
        ...
        public static void ParametersToUI(Form1 fm)
        {
            ...
            {
                // 电池过压值=BusVolt（写入值） × 0.0653 × 255 / 5
                fm.myTextBoxNUM_BatteryMaximumVoltage.Text = (Convert.ToDouble(Form1.gss_BatteryMaximumVoltage_36V) * 5 / 255 / 0.0653).ToString("0.0");
                // 电池欠压值=BusVolt（写入值） × 0.0653 × 255 / 5
                fm.myTextBoxNUM_BatteryMinimumVoltage.Text = (Convert.ToDouble(Form1.gss_BatteryMinimumVoltage_36V) * 5 / 255 / 0.0653).ToString("0.0");
            }
            // 限速线分压系数=写入值 × 10
